...
;;; 
;;;     File:  "stripped all-edges"
;;;   Module:  "drivers;timing:"
;;;  Version:   February 1991

;; file created 1/24/95 as a straight copy of [drivers;chart:all edges:loops]
...
(defun initiate-test-protocol ()
  ...
  (t1/after-scan (scan-next-position)))

(defun t1/dispatch (position &optional caller)
  ...
  (let ((status (pos-assessed? position)))
    (if status
      (ecase status
        ...
        (:edges-done
         (t1/scan&continue (chart-position-after position)))
        (:completed&traversed
         (t1/do-edges (pos-terminal position)
                      (chart-position-after position) position))
        (:word-fsas-done
         (t1/compete-traverse&continue (pos-terminal position) position))
        (:scanned (t1/after-scan position)))
      (else
        (t1/scan&continue position)))))


(defun t1/scan&continue (next-position)
  ...
  (let ((position-scanned (scan-next-position)))
    ...
    (t1/after-scan next-position)))


(defun t1/after-scan (position)
  ...
  (let ((word (pos-terminal position)))
    ...
    (let ((position-after-fsa
      ...
      (if position-after-fsa
        (then
          (t1/do-pending-edges position) ;; added because it fixed a bug in edges fsa case
          (t1/dispatch position-after-fsa))
        (t1/compete-traverse&continue word position)))))



(defun t1/compete-traverse&continue (word position)
  ...
  (let ((next-position (chart-position-after position)))
    ...
    (t1/do-edges word position next-position)))



(defun t1/do-edges (word position next-position)
  (let ((edges-over-word
...
(defun t1/do-pending-edges (position)
  ...
  (let ((index-of-next-edge-to-check *edge-just-checked*)
        ...
        edge )
    (tr :ae/t1/do-pending-edges position)
