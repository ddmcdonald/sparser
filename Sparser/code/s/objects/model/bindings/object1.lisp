     ...
     (let ((bindings (indiv-binds individual)))
       ...
       (when bindings
         (let ((binding
                ...
                ;(dolist (b bindings)
                ;  (format t "~&Binding variable = ~A~%" (binding-variable b))
                ;  (setq v2 (binding-variable b))
                ;  (when (eq variable (binding-variable b))
                ;    (return b)))
                ))
           (when binding
