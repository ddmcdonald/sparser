...
Based on pdfminer pdf2txt with modifications by Laurel to specify python3 and allow outputting multiple files to a directory when given multiple files, as well as allowing output of plain .txt files
example for set of pdfs:
python3 pdf2txt_mod.py ~/projects/r3/corpus/covid/pdfs/*.pdf -D ~/projects/r3/corpus/covid/txt/
single pdf to text example:
python3 pdf2txt_mod.py  ~/projects/r3/corpus/covid/pdfs/AP-200220-0772.pdf -o ~/projects/r3/corpus/covid/txt/AP-200220-0772_COVID_lessons_SARSMERS.txt
"""
import argparse
import logging
import sys
...
def extract_text(files=[], outfile='-',
                 ...
                 output_dir=None, debug=False, disable_caching=False,directory=False,
                 **kwargs):
    if not files:
    ...
    for fname in files:
        if directory:
            dirname, fullname = os.path.split(fname)
            if output_type=='text':
                out_ext ='.txt'
            else:
                out_ext='.'+output_type
            outfile = os.path.join(directory,os.path.splitext(fullname)[0]+out_ext)
            outfp = open(outfile, "wb")
...
def maketheparser():
    ...
    output_params.add_argument(
        "--directory", "-D", default=False,
        help="Location to store the set of output text files")
    return parser
